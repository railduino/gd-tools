...
var Describe = `The mediawiki command prepares a MediaWiki instance for deployment.

MediaWiki is a free and open source wiki software package written in PHP
and using a backend database (typically MySQL/MariaDB).

Homepage ...: https://www.mediawiki.org/
Releases ...: https://www.mediawiki.org/wiki/Download
Installation: https://www.mediawiki.org/wiki/Manual:Installing_MediaWiki
...
var Command = &cli.Command{
	Name:        "mediawiki",
	Usage:       "Prepare a MediaWiki instance for deployment",
	...
	Flags: []cli.Flag{
		...
		&cli.BoolFlag{
			Name:  "update",
			Usage: "update an existing MediaWiki instance",
		},
		&cli.StringFlag{
			Name:  "password",
		...
		&cli.StringFlag{
			Name:  "admin-name",
			Usage: "name of MediaWiki admin",
			Value: "WikiAdmin",
		},
		&cli.StringFlag{
			Name:  "admin-email",
			Usage: "email address of MediaWiki admin",
		},
		&cli.StringFlag{
			Name:  "admin-pswd",
			Usage: "password of MediaWiki admin",
		},
		&cli.StringFlag{
			Name:  "title",
			Usage: "wiki title (sitename); default is <host>.<domain>",
		},
		&cli.StringSliceFlag{
			Name:  "alias",
...
func Run(c *cli.Context) error {
	...
	if c.NArg() != 2 {
		return fmt.Errorf("Usage: gdt mediawiki <host> <domain>")
	}
	host := c.Args().Get(0)
	domain := c.Args().Get(1)
	...
	download, err := agent.GetDownload("mediawiki")
	if err != nil {
		return err
	}
	if download.Directory == "" {
		return fmt.Errorf("missing Directory in mediawiki download")
	}
	...
	if entry.FQDN() == cfg.FQDN() {
		return fmt.Errorf("cannot use the server name for MediaWiki")
	}

	// Default Title if empty
	if entry.Title == "" {
		entry.Title = entry.FQDN()
	}
	...
	if c.Bool("update") {
		...
		cfg.Sayf("MediaWiki '%s' was updated", entry.FQDN())
		return nil
	}
	...
	for _, check := range list.Entries {
		...
		if check.Number == entry.Number {
			...
			if entry.Number > config.LastCMS {
				return fmt.Errorf("no more MediaWiki, please")
			}
		}
	}
